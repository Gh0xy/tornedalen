...
export class ReplyToPostComponent implements OnInit {
  ...
  resetForm() {
    console.log('resetForm called');  // Lägg till denna rad för att kontrollera om metoden anropas
    this.subject = '';
    this.author = '';
    this.email = '';
    this.content = '';
  ...
  submitReply() {
    console.log('submitReply called');  // Lägg till denna rad för att kontrollera om metoden anropas
    if (this.postToReply && this.replyContent && this.postId) {
      ...
      // Lägg till svaret i den aktuella postens replies-array
      this.postToReply.replies = this.postToReply.replies || [];
      ...
      this.postService.addReplyToPost(this.postId, replyPost).subscribe(
        post => {
          this.replyContent = '';
          this.resetForm();
        },
